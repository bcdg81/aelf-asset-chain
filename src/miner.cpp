...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AelfATMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(chainparams, shared_pblock, true, NULL))
        return error("AelfATMiner: ProcessNewBlock, block not accepted");
...
void static BitcoinMiner(const CChainParams& chainparams)
{
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in AelfATMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running AelfATMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("AelfATMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("AelfATMiner runtime error: %s\n", e.what());
        return;
    }
}
...
void static StakeMiner(CWallet *pwallet, const CChainParams& chainparams){
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in StakeMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
